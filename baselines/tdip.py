...
Put all necessary code for the T-DIP baseline here, 
...
class ReconstructionTDIP:
    def __init__(self, param):
        self.param = param
        self.dtype = torch.cuda.FloatTensor

        self.decoder = Decoder(**vars(param.decoder)).type(self.dtype)

        if param.trajectory.type == "helix":
            self.trajectory = self.init_helix_trajectory().type(self.dtype)
    ...
    def evaluate_npy(self, indices):
        return self.evaluate_abs(indices).detach().cpu().numpy().squeeze()
    ...
    def train(self, kspace, sensitivities, mask):
        ...
        while i <= num_epochs: # iterate over epochs
            ...
            if i%self.param.experiment.model_save_frequency == 0 or i==self.param.hp.num_iter:
                ...
                # save reconstruction
                with torch.no_grad():
                    reconstruction = torch.cat([self.evaluate(k) for k in range(self.param.data.Nk)], dim=0)
            ...
            if i%self.param.experiment.video_evaluation_frequency == 0 or i==self.param.hp.num_iter-1:
                with torch.no_grad():
                    imgs = torch.stack([self.evaluate_abs(k).detach().cpu() for k in range(self.param.data.Nk)], dim=0)
